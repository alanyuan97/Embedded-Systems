...
def norm_read(org,dis):
    amp_dis = np.multiply(org,dis)
    max_amp_dis = np.max(amp_dis)
    min_amp_dis = np.min(amp_dis)

    # Normalize data
    numerator = np.subtract(amp_dis,min_amp_dis)
    norm = 255* (np.divide(numerator,max_amp_dis-min_amp_dis))
    data = np.array(norm)
    print("Amplified reading: {} ".format(amp_dis))
    sorted_data = np.sort(data)
    return sorted_data.astype(int),np.mean(sorted_data),np.var(sorted_data)
...
def start_reading():
    ...
    for _ in range(50):
        ...
        reds.append(rgbdata[3] * 256 + rgbdata[2])
        greens.append(rgbdata[5] * 256 + rgbdata[4])
        blues.append(rgbdata[7] * 256 + rgbdata[6])
    ...
    red_int,rmean,rvar = norm_read(reds,diss)
    green_int,gmean,gvar= norm_read(greens,diss)
    blue_int,bmean,bvar= norm_read(blues,diss)

    print("Red-Data: {0} Mean {1} Var {2} ".format(red_int,rmean,rvar))
    print("Green-Data: {0} Mean {1} Var {2}".format(green_int,gmean,gvar))
    print("Blue-Data: {0} Mean {1} Var {2}".format(blue_int,bmean,bvar))

    ups = len(red_int[red_int>UPTHRS])
    lws = len(red_int[red_int<LWTHRS])*PENALTY

    score = base_reward + ups - lws

    print(f"Score : {score}")

    print(f"Red Passed: {len(red_int[red_int>threshold])} \n")
    
    lblue.off()

    # if (len(red_int[red_int>threshold])>=4):
    if (score>=20):
        lgreen.on()
        y=json.dumps("good")
